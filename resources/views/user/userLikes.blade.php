<x-layout title="My 名言 | 「いいね」した投稿一覧">
    ...
    <div class="flex justify-center w-9/12 mx-auto py-1 bg-gray-300 rounded-2xl mb-4">
        ...
        <h2 class="ml-1 items-center">{{ $user->name }}&ensp;|&ensp;投稿一覧</h2>
    </div>


    <p class="text-center">{{ $user->name }} さんがいいねをした投稿一覧です</p>
    <!-- post本体 -->
    <div class="w-11/12  mx-auto border-2 rounded-lg pt-6 mb-12">


    @if ($postLikes->count() === 0)
    <p class="text-gray-500 text-center italic mb-4">いいねした投稿はありません</p>
    @endif

    @foreach ($postLikes as $postLike)
        <div class="mb-8 border-b relative">
            <p class="w-10/12 mx-auto mb-6 {{ $postLike->post->font }}">{!! nl2br(e($postLike->post->content)) !!}</p>
            <div class="flex justify-center align-center mb-6">
                @auth
                    @php
                    $postService = new App\Services\PostService();
                    $hasLikedByLoginUser = $postService->hasLikedByUser($postLike->post_id, $user->id)
                    @endphp
                ...
                @endguest
                <p class="ml-2">{{ $postLike->post->num_of_likes }}</p>
            </div>
            <form class="absolute inset-0" action="{{ route('post.detail', ['id' => $postLike->post_id]) }}" method="GET">
